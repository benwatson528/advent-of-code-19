...
class AmplificationCircuitTest extends FunSuite {
  test("maxSignal 4,3,2,1,0") {
    val input = "3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0".split(",").map(_.toInt).toIndexedSeq
    assert(AmplificationCircuit.maxSignal(input) === 43210)
  }

  test("maxSignal 0,1,2,3,4") {
    val input = "3,23,3,24,1002,24,10,24,1002,23,-1,23,101,5,23,23,1,24,23,23,4,23,99,0,0".split(",").map(_.toInt)
        .toIndexedSeq
  ...
  test("sumNumbers real") {
    val input = Source.fromResource("day07/input.txt").getLines().next().split(",").map(_.toInt).toIndexedSeq
