...
class SunnyWithAChanceOfAsteroidsTest extends FunSuite {
  ...
  test("runProgram complex opcode 3 instruction") {
    ...
    assert(SunnyWithAChanceOfAsteroids.runProgram(input, List(1)) === 1)
  }

  test("runProgram real part 1") {
    val input = splitInput(Source.fromResource("day05/input.txt").getLines().next())
    assert(SunnyWithAChanceOfAsteroids.runProgram(input, List(1)) === 10987514)
  }
  ...
  test("runProgram jump (rule 5) position - output 0 if the input is 0 else 1") {
    ...
    assert(SunnyWithAChanceOfAsteroids.runProgram(input, List(0)) === 0)
    assert(SunnyWithAChanceOfAsteroids.runProgram(input, List(10)) === 1)
  }
  ...
  test("runProgram less than (rule 7) position - ouputs 1 if less than 8, else 0") {
    ...
    assert(SunnyWithAChanceOfAsteroids.runProgram(input, List(7)) === 1)
    assert(SunnyWithAChanceOfAsteroids.runProgram(input, List(9)) === 0)
  }
  ...
  test("runProgram equals (rule 8) position comparison - ouputs 1 if equal to 8, else 0") {
    ...
    assert(SunnyWithAChanceOfAsteroids.runProgram(input, List(8)) === 1)
    assert(SunnyWithAChanceOfAsteroids.runProgram(input, List(15)) === 0)
  }
  ...
  test("runProgram real part 2") {
    val input = splitInput(Source.fromResource("day05/input.txt").getLines().next())
    assert(SunnyWithAChanceOfAsteroids.runProgram(input, List(5)) === 14195011)
  }
