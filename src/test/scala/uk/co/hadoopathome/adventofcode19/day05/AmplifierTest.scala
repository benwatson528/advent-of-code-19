...
class AmplifierTest extends FunSuite {
  ...
  test("runProgram complex opcode 3 instruction") {
    ...
    assert(new Amplifier(input, 1).runUntilCompletion() === 1)
  }

  test("runProgram real part 1") {
    val input = splitInput(Source.fromResource("day05/input.txt").getLines().next())
    assert(new Amplifier(input, 1).runUntilCompletion() === 10987514)
  }
  ...
  test("runProgram jump (rule 5) position - output 0 if the input is 0 else 1") {
    ...
    assert(new Amplifier(input, 0).runUntilCompletion() === 0)
    assert(new Amplifier(input, 10).runUntilCompletion() === 1)
  }
  ...
  test("runProgram less than (rule 7) position - ouputs 1 if less than 8, else 0") {
    ...
    assert(new Amplifier(input, 7).runUntilCompletion() === 1)
    assert(new Amplifier(input, 9).runUntilCompletion() === 0)
  }
  ...
  test("runProgram equals (rule 8) position comparison - ouputs 1 if equal to 8, else 0") {
    ...
    assert(new Amplifier(input, 8).runUntilCompletion() === 1)
    assert(new Amplifier(input, 15).runUntilCompletion() === 0)
  }
  ...
  test("runProgram real part 2") {
    val input = splitInput(Source.fromResource("day05/input.txt").getLines().next())
    assert(new Amplifier(input, 5).runUntilCompletion() === 14195011)
  }
