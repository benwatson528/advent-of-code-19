...
class AmplifierTest extends FunSuite {
  ...
  test("runProgram complex opcode 3 instruction") {
    ...
    assert(new Amplifier(input).runUntilCompletion(1) === 1)
  }

  test("runProgram real part 1") {
    val input = splitInput(Source.fromResource("day05/input.txt").getLines().next())
    assert(new Amplifier(input).runUntilCompletion(1) === 10987514)
  }
  ...
  test("runProgram jump (rule 5) position - output 0 if the input is 0 else 1") {
    ...
    assert(new Amplifier(input).runUntilCompletion(0) === 0)
    assert(new Amplifier(input).runUntilCompletion(10) === 1)
  }
  ...
  test("runProgram less than (rule 7) position - ouputs 1 if less than 8, else 0") {
    ...
    assert(new Amplifier(input).runUntilCompletion(7) === 1)
    assert(new Amplifier(input).runUntilCompletion(9) === 0)
  }
  ...
  test("runProgram equals (rule 8) position comparison - ouputs 1 if equal to 8, else 0") {
    ...
    assert(new Amplifier(input).runUntilCompletion(8) === 1)
    assert(new Amplifier(input).runUntilCompletion(15) === 0)
  }
  ...
  test("runProgram real part 2") {
    val input = splitInput(Source.fromResource("day05/input.txt").getLines().next())
    assert(new Amplifier(input).runUntilCompletion(5) === 14195011)
  }
